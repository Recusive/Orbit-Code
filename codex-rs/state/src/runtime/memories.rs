...
mod tests {
    ...
    async fn stage1_claim_skips_when_up_to_date() {
        ...
        let metadata = test_thread_metadata(&orbit_code_home, thread_id, orbit_code_home.join("a"));
        runtime
    ...
    async fn stage1_running_stale_can_be_stolen_but_fresh_running_is_skipped() {
        ...
        let owner_a = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let owner_b = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let cwd = orbit_code_home.join("workspace");
        runtime
            .upsert_thread(&test_thread_metadata(&orbit_code_home, thread_id, cwd))
            .await
            .expect("upsert thread");
    ...
    async fn stage1_concurrent_claim_for_same_thread_is_conflict_safe() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &orbit_code_home,
                thread_id,
                orbit_code_home.join("workspace"),
            ))
            .await
            .expect("upsert thread");
    ...
    async fn stage1_concurrent_claims_respect_running_cap() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &orbit_code_home,
                thread_a,
                orbit_code_home.join("workspace-a"),
            ))
            .await
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &orbit_code_home,
                thread_b,
                orbit_code_home.join("workspace-b"),
            ))
            .await
    ...
    async fn claim_stage1_jobs_filters_by_age_idle_and_current_thread() {
        ...
        let mut just_under_idle = test_thread_metadata(
            ...
            orbit_code_home.join("just-under-idle"),
        );
        ...
        let mut eligible_idle = test_thread_metadata(
            ...
            orbit_code_home.join("eligible-idle"),
        );
        ...
        let mut old =
            test_thread_metadata(&orbit_code_home, old_thread_id, orbit_code_home.join("old"));
    ...
    async fn claim_stage1_jobs_prefilters_threads_with_up_to_date_memory() {
        ...
        let mut up_to_date = test_thread_metadata(
            ...
            orbit_code_home.join("up-to-date"),
        );
    ...
    async fn claim_stage1_jobs_enforces_global_running_cap() {
        ...
        for idx in 0..total_candidates {
            ...
            let mut metadata = test_thread_metadata(
                &orbit_code_home,
                thread_id,
                orbit_code_home.join(format!("thread-{idx}")),
            );
...
WHERE kind = 'memory_stage1'
  ...
  AND lease_until > ?
    ...
    async fn list_stage1_outputs_for_global_returns_latest_outputs() {
        ...
        assert_eq!(outputs[0].cwd, orbit_code_home.join("workspace-b"));
        ...
        assert_eq!(outputs[1].cwd, orbit_code_home.join("workspace-a"));
    ...
    async fn list_stage1_outputs_for_global_skips_empty_payloads() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &orbit_code_home,
                thread_id_non_empty,
                orbit_code_home.join("workspace-non-empty"),
            ))
            .await
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &orbit_code_home,
                thread_id_empty,
                orbit_code_home.join("workspace-empty"),
            ))
            .await
...
VALUES (?, ?, ?, ?, ?)
        ...
        assert_eq!(outputs[0].cwd, orbit_code_home.join("workspace-non-empty"));
    ...
    async fn get_phase2_input_selection_reports_added_retained_and_removed_rows() {
        ...
        assert_eq!(
            ...
            orbit_code_home.join(format!("rollout-{thread_id_c}.jsonl"))
        );
        ...
        for (thread_id, updated_at) in [(thread_id_enabled, 100), (thread_id_polluted, 101)] {
            runtime
                .upsert_thread(&test_thread_metadata(
                    &orbit_code_home,
                    thread_id,
                    orbit_code_home.join(thread_id.to_string()),
                ))
                .await
                .expect("upsert thread");
