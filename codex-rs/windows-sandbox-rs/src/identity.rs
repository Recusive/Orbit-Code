...
pub fn sandbox_setup_is_complete(orbit_code_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(orbit_code_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(orbit_code_home), Ok(Some(users)) if users.version_matches())
}

fn load_marker(orbit_code_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(orbit_code_home);
...
fn load_users(orbit_code_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(orbit_code_home);
...
fn select_identity(policy: &SandboxPolicy, orbit_code_home: &Path) -> Result<Option<SandboxIdentity>> {
    let _marker = match load_marker(orbit_code_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(orbit_code_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(orbit_code_home);
    let needed_read = gather_read_roots(command_cwd, policy, orbit_code_home);
    let needed_write = gather_write_roots(policy, policy_cwd, command_cwd, env_map);
    // NOTE: Do not add ORBIT_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match load_marker(orbit_code_home)? {
        Some(marker) if marker.version_matches() => {
            _existing_marker = Some(marker.clone());
            let selected = select_identity(policy, orbit_code_home)?;
            if selected.is_none() {
                setup_reason =
    ...
    if identity.is_none() {
        ...
        identity = select_identity(policy, orbit_code_home)?;
    }
    // Always refresh ACLs (non-elevated) for current roots via the setup binary.
    crate::setup::run_setup_refresh(policy, policy_cwd, command_cwd, env_map, orbit_code_home)?;
