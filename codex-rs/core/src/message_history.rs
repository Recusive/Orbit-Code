...
//!
//! The history is stored at `~/.orbit/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.orbit`.
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.orbit/history.jsonl` and ensure the parent directory exists.
