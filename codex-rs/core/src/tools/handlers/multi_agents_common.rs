//! Shared helpers for multi-agent tool handlers.
//!
//! TODO(upstream-0.118.0): restore upstream path-based agent routing once
//! `orbit-code-protocol` regains `agent_path` support and `AgentControl` grows
//! the matching registry APIs on this branch.
...
pub(crate) fn collab_spawn_error(err: CodexErr) -> FunctionCallError {
    match err {
        ...
        other => FunctionCallError::RespondToModel(format!("collab spawn failed: {other}")),
    }
}
...
pub(crate) fn collab_agent_error(agent_id: ThreadId, err: CodexErr) -> FunctionCallError {
    match err {
        ...
        other => FunctionCallError::RespondToModel(format!("collab tool failed: {other}")),
    }
}

pub(crate) fn thread_spawn_source(
    parent_thread_id: ThreadId,
    _parent_session_source: &SessionSource,
    depth: i32,
    agent_role: Option<&str>,
    _task_name: Option<String>,
) -> Result<SessionSource, FunctionCallError> {
    // TODO(upstream-0.118.0): map `task_name` onto `agent_path` once the protocol on
    // this branch includes that field again.
