...
use crate::tools::handlers::multi_agents_common::input_preview;
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        if args.task_name.trim().is_empty() {
            return Err(FunctionCallError::RespondToModel(
                "task_name must be non-empty".to_string(),
            ));
        }
        ...
        Ok(SpawnAgentResult {
            // TODO(upstream-0.118.0): switch this back to task-name-only addressing once
            // path-based agent resolution is available on this branch.
