//! Startup-time sync helpers for curated and remote plugins.
...
use orbit_code_otel::metrics::names::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use orbit_code_otel::metrics::names::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
pub(super) fn start_startup_remote_plugin_sync_once(
    ...
    tokio::spawn(async move {
        ...
        match manager
            // TODO(upstream-0.118.0): restore the upstream additive-only flag once
            // `PluginsManager::sync_plugins_from_remote` grows that parameter here.
