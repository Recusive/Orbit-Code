...
fn create_seatbelt_args_with_read_only_git_and_orbit_code_subpaths() {
    ...
    // top-level .git and .orbit directories and one without them.
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
    ...
    // write to .orbit/config.toml in the vulnerable root.
    ...
    // - write access to WRITABLE_ROOT_0 (but not its .git or .orbit), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
"#,
    ...
    // Verify that .orbit/config.toml cannot be modified under the generated
    // Seatbelt policy.
    ...
    // Verify that writing a file to the folder containing .git and .orbit is allowed.
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // `.orbit` checks are done properly for cwd.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: Default::default(),
        network_access: false,
...
struct PopulatedTmp {
    /// Path containing a .git and .orbit subfolder.
    ...
    /// ran `git commit` themselves, or modified .orbit/config.toml to
    ...
    /// Path without .git or .orbit subfolders.
    empty_root: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".orbit")).expect("create .orbit");
    fs::write(
        vulnerable_root.join(".orbit").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .orbit/config.toml");
    ...
    let dot_orbit_code_canonical = vulnerable_root_canonical.join(".orbit");
