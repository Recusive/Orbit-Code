...
impl OllamaClient {
    ...
    async fn try_from_provider_with_base_url(base_url: &str) -> io::Result<Self> {
        let provider = orbit_code_core::create_oss_provider_with_base_url(
            base_url,
            orbit_code_core::WireApi::Responses,
        );
        Self::try_from_provider(&provider).await
    }
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(orbit_code_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
