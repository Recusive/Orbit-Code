...
- "Where should I create this skill? If you do not have a preference, I will place it in `$ORBIT_HOME/skills` (or `~/.orbit/skills` when `ORBIT_HOME` is unset) so Codex can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `$ORBIT_HOME/skills`; when `ORBIT_HOME` is unset, fall back to `~/.orbit/skills` so the skill is auto-discovered.
...
```bash
scripts/init_skill.py my-skill --path "${ORBIT_HOME:-$HOME/.orbit}/skills"
scripts/init_skill.py my-skill --path "${ORBIT_HOME:-$HOME/.orbit}/skills" --resources scripts,references
