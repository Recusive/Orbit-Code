...
use orbit_code_utils_output_truncation::truncate_middle_chars;
...
use orbit_code_protocol::protocol::GitInfo as ProtocolGitInfo;
...
use orbit_code_utils_path as path_utils;
...
impl RolloutRecorder {
    ...
    async fn list_threads_with_db_fallback(
        ...
        let codex_home = config.orbit_code_home();
...
fn precompute_log_file_info(
    ...
    let mut dir = config.orbit_code_home().to_path_buf();
...
async fn write_session_meta(
    ...
    let git_info = collect_git_info(cwd).await.map(|info| ProtocolGitInfo {
        commit_hash: info.commit_hash.map(|sha| sha.0),
        branch: info.branch,
