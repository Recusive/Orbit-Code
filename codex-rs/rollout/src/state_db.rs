...
use orbit_code_utils_path::normalize_for_path_comparison;
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = orbit_code_state::state_db_path(config.sqlite_home());
...
pub async fn open_if_present(codex_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = orbit_code_state::state_db_path(codex_home);
...
pub async fn list_thread_ids_db(
    ...
    let ctx = context?;
    if ctx.orbit_code_home() != codex_home {
        warn!(
