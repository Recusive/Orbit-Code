...
pub async fn backfill_sessions(
    ...
    let sessions_root = config.orbit_code_home().join(SESSIONS_SUBDIR);
    let archived_root = config.orbit_code_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.orbit_code_home(), &path),
                    path,
                    archived,
                }));
            }
