...
impl AuthModeWidget {
    ...
    fn save_anthropic_api_key(&mut self, api_key: String) {
        ...
        match save_auth_v2(
            ...
            Ok(()) => {
                self.error = None;
                self.login_status = LoginStatus::AuthMode(AuthMode::AnthropicApiKey);
                self.auth_manager.reload();
                *self.sign_in_state.write().unwrap() = SignInState::AnthropicApiKeyConfigured;
            }
            Err(err) => {
    ...
    fn render_anthropic_api_key_entry(
        ...
        if let Some(err) = &self.error {
            all_lines.push("".into());
            all_lines.push(err.as_str().red().into());
        }
    ...
    fn start_anthropic_oauth(&mut self) {
        ...
        let (auth_url, verifier) = match orbit_code_login::anthropic_authorize_url(
            ...
            Ok(result) => result,
            Err(e) => {
                self.error = Some(format!("Failed to generate OAuth URL: {e}"));
                self.request_frame.schedule_frame();
                return;
            }
        };
    ...
    fn exchange_anthropic_oauth_code(&mut self, code_with_state: String, verifier: String) {
        // Show immediate feedback before the async exchange starts
        {
            let mut guard = self.sign_in_state.write().unwrap();
            if let SignInState::AnthropicOAuthCodeEntry(state) = &mut *guard {
                state.error = Some("Exchanging code...".to_string());
            }
        }
        self.request_frame.schedule_frame();
    ...
    fn render_anthropic_oauth_code_entry(
        ...
        let err = self.error.as_deref().or(state.error.as_deref());
        if let Some(err) = err {
            lines.push("".into());
