# codex-rs/tui/frames/dots/

This file applies to `codex-rs/tui/frames/dots/` and its descendants. Follow the repo root `AGENTS.md` first, then use the local rules below when you edit this subtree.
...
Dot-based ASCII animation frames.

### What this folder does

Contains 36 ASCII art text frames (`frame_1.txt` through `frame_36.txt`) for the "dots" animation variant of the TUI loading spinner. Uses dot/braille characters for the animation style.

### What it plugs into

- **src/frames.rs**: Embedded at compile time via `include_str!()` into the `FRAMES_DOTS` constant array.
