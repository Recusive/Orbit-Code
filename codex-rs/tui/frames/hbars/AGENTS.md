...
This file applies to `codex-rs/tui/frames/hbars/` and its descendants. Follow the repo root `AGENTS.md` first, then use the local rules below when you edit this subtree.
...
Horizontal bar ASCII animation frames.

### What this folder does

Contains 36 ASCII art text frames (`frame_1.txt` through `frame_36.txt`) for the "hbars" animation variant of the TUI loading spinner. Uses horizontal bar characters for the animation style.

### What it plugs into

- **src/frames.rs**: Embedded at compile time via `include_str!()` into the `FRAMES_HBARS` constant array.
