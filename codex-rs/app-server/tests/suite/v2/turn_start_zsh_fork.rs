...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&orbit_code_home, &workspace).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(orbit_code_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(orbit_code_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(orbit_code_app_server_protocol::AskForApproval::UnlessTrusted),
            sandbox_policy: Some(
...
async fn create_zsh_test_mcp_process(orbit_code_home: &Path, zdotdir: &Path) -> Result<McpProcess> {
    let zdotdir = zdotdir.to_string_lossy().into_owned();
    McpProcess::new_with_env(orbit_code_home, &[("ZDOTDIR", Some(zdotdir.as_str()))]).await
}
