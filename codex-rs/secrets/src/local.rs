...
impl LocalSecretsBackend {
    pub fn new(orbit_code_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            orbit_code_home,
            keyring_store,
        }
    ...
    fn secrets_dir(&self) -> PathBuf {
        self.orbit_code_home.join("secrets")
    }
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.orbit_code_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(orbit_code_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let orbit_code_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(orbit_code_home.path());
        keyring.set_error(
            &account,
