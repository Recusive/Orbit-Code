...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = orbit_code_command(orbit_code_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = orbit_code_command(orbit_code_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = orbit_code_command(orbit_code_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!orbit_code_home.path().join(".credentials.json").exists());
    assert!(!orbit_code_home.path().join(".env").exists());
