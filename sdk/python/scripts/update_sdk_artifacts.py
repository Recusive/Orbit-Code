...
def staged_runtime_bin_path(root: Path) -> Path:
    return root / "src" / "orbit_code_cli_bin" / "bin" / runtime_binary_name()
...
def _rewrite_sdk_runtime_dependency(pyproject_text: str, runtime_version: str) -> str:
    ...
    raw_items = [item for item in raw_items if "orbit-code-cli-bin" not in item]
    raw_items.append(f'"orbit-code-cli-bin=={runtime_version}"')
...
def stage_python_sdk_package(
    ...
    sdk_bin_dir = staging_dir / "src" / "orbit_code_app_server" / "bin"
...
def generate_v2_all() -> None:
    out_path = sdk_root() / "src" / "orbit_code_app_server" / "generated" / "v2_all.py"
...
def _notification_specs() -> list[tuple[str, str]]:
    ...
    generated_source = (
        sdk_root() / "src" / "orbit_code_app_server" / "generated" / "v2_all.py"
    ).read_text()
...
def _render_async_orbit_code_block(
    thread_start_fields: list[PublicFieldSpec],
    thread_list_fields: list[PublicFieldSpec],
    resume_fields: list[PublicFieldSpec],
    fork_fields: list[PublicFieldSpec],
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "orbit_code_app_server" / "api.py"
    ...
    thread_start_fields = _load_public_fields(
        "orbit_code_app_server.generated.v2_all",
        "ThreadStartParams",
    )
    ...
    source = _replace_generated_block(
        ...
        _render_async_orbit_code_block(
            thread_start_fields,
            thread_list_fields,
            thread_resume_fields,
            thread_fork_fields,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_sdk_parser.add_argument(
        "--runtime-version",
        required=True,
        help="Pinned orbit-code-cli-bin version for the staged SDK package",
    )
    stage_sdk_parser.add_argument(
        "--sdk-version",
